        ...
        if(isset($_POST['altajuego'])){
            ...
            if(empty($_POST['nombrejuego']))
                ...
                if(!validar_juego($_POST['nombrejuego']))
                    ...
                    $fecha= date('Y-m-d');
                    $ids_hoy= obtener_id_juegos_hoy($fecha, $_SESSION['turno'], $db);
                    if(mysqli_num_rows($ids_hoy)==0){
                        if(alta_juego(($mayor+1), strtolower(eliminar_tildes(trim($_POST['nombrejuego']))), $fecha, $_SESSION['turno'], $db))
                            echo "<p class=trece>Juego dado de alta con éxito:<p/><p class=catorce>- Id juego: ".($mayor+1)."<br/>- Juego: ".strtolower(eliminar_tildes(trim($_POST['nombrejuego'])))."<br/>- Fecha alta: ".date("d/m/Y", strtotime($fecha))."<br/>- Turno: ".$_SESSION['turno']."</p>";
                    }
                    elseif(mysqli_num_rows($ids_hoy)==5)
                        echo "<span class=once>Solo se puede dar de alta un máximo de 5 juegos por día.</span>";
                    else{
                        $fila= obtener_juegos($fecha, $_SESSION['turno'], $db);
                        $juegos= array();
