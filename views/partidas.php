...
<html>
	<head>
		...
		<link href="css/estilos-partidas.css" rel="stylesheet" type="text/css" />
		<title>Otras partidas</title>
	</head>
	<body>
		<header>
			<div class= "tres">
		...
		<nav>
			<ul class="nueve">
				...
				<li class="partidas"><a href="controller-partidas.php">Otras partidas</a></li>
			</ul>
		</nav>
		<main>
			<section>
				<article>
					<div class="catorce">
						<h1>Otras partidas</h1>
						<p>Seleccione fecha inicio, fecha fin y luego pulse 'Ver partidas' para ver las partidas que se registraron entre las fechas que ha seleccionado.</p>
						<img src="imagenes/once.jpg" alt="Imagen Gaming" width="200px" >
                        <form method="post">
                            ...
                            <input type="submit" name="partidas" value="Ver partidas" class="diez">
                        </form>
						<?php
							if(isset($_POST['partidas'])){
								if(empty($_POST['desde']))
									echo "<span class=doce>Introduzca una fecha de inicio.</span>";
								elseif(empty($_POST['hasta']))
									echo "<span class=doce>Introduzca una fecha de fin.</span>";
								elseif($_POST['desde']>$_POST['hasta'])
									echo "<span class=doce>La fecha de fin debe ser mayor a la fecha de inicio.</span>";
								else{
									if(mysqli_num_rows($fila)==0)
										echo "<span class=doce>Entre ".date("d/m/Y", strtotime($_POST['desde']))." y ".date("d/m/Y", strtotime($_POST['hasta']))." no hay partidas.</span>";
									else{
										echo
										"<div class=trece>
										<table>
